...
**Statement form.** `RD211` is the division-free *gate form* of rational-derivedness (design choice: verification-friendly restatement). The kernel contains the quartic itself (`quartic211`, with `quartic211_expand`), the identification of the two gates as the derivative discriminants (`gate1_is_disc`, `gate2_is_disc`), and the explicit rational splittings of both derivatives under the gates (`dq1_splits`, `dq2_splits`). Two links are held outside the kernel and are covered by the statement-fidelity gate instead: that the expanded forms `dq1`, `dq2` are the derivatives of `quartic211` (checked by symbolic differentiation in `scripts/sage/wp1_fidelity.sage`, md5 `5cbcc1b4c4a70acd58e8e967c873852f`), and the converse implication *splitting ⇒ gate* (elementary discriminant theory, used nowhere in the proof). The frozen theorem statement is the gate form.
