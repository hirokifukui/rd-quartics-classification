...
1. **The gap** (`Gap/`): kernel certificates that the published proof of Theorem 7 contains a circularity and a quantifier jump, exhibited by an explicit quartic over two base fields (`BMThm7Gap.lean`, `BMThm7Transcript.lean`; adjudication record in `Gap/ADJUDICATION.md`, pre-registration frozen before any Lean build in `Gap/STEPS.md`).
