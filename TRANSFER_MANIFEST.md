...
## 査読対応記録の所在（2026-08-03 整理）

2026-08-02〜03 の外部査読対応の詳細記録は `archive/MANIFEST_REVIEW_RESPONSES_202608.md` に逐語移管
（実質の弧は `REVIEW_HISTORY.md`）。**現在 authoritative な状態**: 凍結 = release **v1.0.2**（README・
CITATION・line-pins と完全同期）／kernel = 137 宣言 checkdecls・主要定理 exact footprint std-3・
開示公理 rank_E576i2 は主線外／CAS = 二系統＋digest 固定コンテナの毎 push 実再計算（43 CHECK）／
CI = build・axiom audit・cert audit・**line-pin audit**・CAS reproduce・deploy 全て実門。
