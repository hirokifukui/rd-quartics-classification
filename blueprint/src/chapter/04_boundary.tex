...
of $K(S)$ is a $K(B)$-combination of the basis $\{1,z,w,zw\}$ [M: standard basis
of the quadratic tower].  Kernel statements about $K(S)$ below are proved in span
...
reach all of $K(S)$ through that basis [M].  Throughout, $\Q(a,b)$ is the base
...
$x_{0}=\tfrac{3\sigma_{1}+r}{8}$ & $C_{+}\colon v^{2}=s_{1n}(s_{1n}-z)$ & $-2Q_{4}$ (norm criterion) & [K] Thm~\ref{multiquad_verdicts} \\
$x_{0}=\tfrac{3\sigma_{1}-r}{8}$ & $C_{-}\colon v^{2}=s_{1n}(s_{1n}+z)$ & $-2Q_{4}$ (norm criterion) & [K] Thm~\ref{multiquad_verdicts} \\
...
$C_{\pm}$: if $\gamma=s_{1n}(s_{1n}\mp z)$ were a square in $K(S)$, its
$K(B)$-norm from $K(B)(z)$, namely
$N(\gamma)=s_{1n}^{2}(s_{1n}^{2}-R_{q})=-8\,s_{1n}^{2}\,Q_{4}$ --- the kernel
identity I2 --- would have to be a square where relevant, which is the statement
that $-2Q_{4}$ is a square [M: norm criterion for quadratic extensions; the
identity itself is K].  The two $C_{\pm}$ covers thus share the single radicand
class $-2Q_{4}$, and $C_{0}$ the class $Q_{4}$: three covers, two square classes,
one sextic.  In normalised form the conjecture asserts that no
...
Since every element of $K(S)$ lies in the span [M: basis of the quadratic tower],
the theorem excludes squares in all of $K(S)$; the kernel boundary is exactly the
span statement, the basis fact is ordinary mathematics.

\begin{proposition}[Geometric integrality of the covers]\label{prop:geomint}
Neither $Q_{4}$ nor $-2Q_{4}$ is a square in $\overline{\Q}(S)$; each cover
$C_{0},C_{\pm}\to S$ is a dominant, geometrically integral degree-two cover away
from its branch locus.
\end{proposition}
...
to $-2Q_{4}$ (same zero divisor).  Dominance and generic degree two are
...
cover form a thin subset [M].  (The application of Hilbert irreducibility: irreducibility of the covers over
$K(S)$ follows from Theorem~\ref{multiquad_verdicts}; dominance, degree two, and
geometric integrality away from the branch locus are Proposition~\ref{prop:geomint}.)
