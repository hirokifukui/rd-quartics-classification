...
\begin{definition}[The cover models]\label{def:cover_models}
Let $U_{B}\subset B$ be the complement of $\{\mathrm{den}=0\}$ and of the
degeneracy divisors (coincident or vanishing roots), and $U\subset S$ its
preimage in the splitting surface; concretely, the splitting surface over the
chart is the affine model
$S_{U}:=\operatorname{Spec}\bigl(\mathcal O(U_{B})[z,w]/(z^{2}-R_{q},\,w^{2}-S_{q})\bigr)$,
and $U$ is its open part over $U_{B}$.  \emph{The covers} are the affine models
$\widetilde C_{0}:=\operatorname{Spec}\bigl(\mathcal O(U)[v]/(v^{2}-Q_{4})\bigr)$
and $C_{\pm}:=\operatorname{Spec}\bigl(\mathcal O(U)[v]/(v^{2}-\gamma_{\pm})\bigr)$,
together with the chart model $C_{0}\colon t^{2}=\sigma_{1}^{2}-4\sigma_{2}$
over $U$.  Throughout, ``cover'' refers to these models: their generic fibres
are the function-field extensions studied below; the branch locus of each is supported on the zero locus of its radicand on
$U$, with ramification exactly at the codimension-one points where the
radicand has odd valuation; and the rational fibre over
$P\in U(\Q)$ is the solution set $\{v\in\Q:\,v^{2}=(\text{radicand})(P)\}$.
\end{definition}
...
\begin{proposition}[Fibre correspondence on the pole-free splitting chart]\label{prop:fibre_correspondence}\lean{BMThm7Fibre.fibre_correspondence}\leanok
...
rational point.  (The kernel hypotheses are exactly these two ---
$\mathrm{den}\neq0$ and $z^{2}=R_{q}$; no further nondegeneracy is needed, so
the formal statement is slightly stronger than the chart discussion requires.)
\end{proposition}
...
(1)~For each $j$, choose a variable --- say $b$ --- in which $g_{j}$ is
nonconstant, and regard $g_{j}$ as a univariate polynomial over $K=k(a)$.
Squarefreeness over any extension of $K$ is detected by
$\gcd_{b}(g_{j},\partial g_{j}/\partial b)$, and the Euclidean algorithm
computing this gcd is unchanged by extension of the coefficient field; in
characteristic zero the derivative is nonzero and an irreducible $g_{j}$ is
coprime to it.  Hence $g_{j}$ stays squarefree over $k'(a)$ for every
algebraic $k'/k$: every geometric component of $g_{j}=0$ occurs with
multiplicity one.  (This self-contained argument is the elementary face of
geometric reducedness over perfect fields, cf.\ G\"ortz--Wedhorn,
\emph{Algebraic Geometry~I}, the section on geometrically reduced schemes;
over an imperfect field one would additionally exclude $p$-th powers.)  (2)~Distinct $g_{i},g_{j}$ are coprime in $k[a,b]$; if they had a common
irreducible factor $h$ over some algebraic extension, the product of the
finitely many Galois conjugates of $h$ (over a normal closure containing its
coefficients) would be a polynomial with coefficients in $k$ dividing both ---
contradicting coprimality over $k$.  So no geometric component is shared
between different $g_{j}$; every geometric component of $f=0$ therefore carries the
...
Scheme-theoretically (Definition~\ref{def:cover_models}): the coordinate ring
$\mathcal O(U_{B})[z,w]/(z^{2}-R_{q},w^{2}-S_{q})$ is generated as an
$\mathcal O(U_{B})$-module by $\{1,z,w,zw\}$ (reduce by the two relations), and
freely so --- a nontrivial $\mathcal O(U_{B})$-relation would specialise to one
in the generic fibre, contradicting the basis property there.  A free module
over the domain $\mathcal O(U_{B})$ is torsion-free, so the ring injects into
its generic localisation, which is the degree-four \emph{field}
$K(B)(\sqrt{R_{q}},\sqrt{S_{q}})$ of Lemma~\ref{lem:biquad_criterion}; a ring
embedding into a field is a domain.  Hence $S_{U}$ is integral, and
geometrically so (the same chain over $\overline{\Q}$, whose hypotheses hold by
Lemma~\ref{lem:sqfree_basechange}) [M].
...
geometrically integral degree-two covers of $U$ (Definition~\ref{def:cover_models};
...
integrality are Proposition~\ref{prop:geomint}.)
