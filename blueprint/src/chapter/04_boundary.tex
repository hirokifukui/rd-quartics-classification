...
covers of $S$.  Point~1 is the $K$-witness $(-51/13,-17/7)$; point~2 is $(1,3)$.  The kernel content of
this subsection is exactly the eight negativity certificates; the specialisation
principle itself (a square in $\Q(a,b)$ is non-negative at every rational point of its
domain) is elementary but unformalised, and the passage to non-squareness in the
function field $\Q(S)$ of the surface is the two-system [MC] verdict, not a kernel
theorem.
...
cover form a thin subset.  Thinness bounds what a generic-point argument can see, not what
...
The reading is an assessment anchored by machine artefacts, not a theorem: within the
