...
$x_{0}=\tfrac{3\sigma_{1}+r}{8}$ & $C_{+}\colon v^{2}=s_{1n}(s_{1n}-z)$ & $-2Q_{4}$ (norm criterion) & [K] Thm~\ref{gamma_verdicts} \\
$x_{0}=\tfrac{3\sigma_{1}-r}{8}$ & $C_{-}\colon v^{2}=s_{1n}(s_{1n}+z)$ & $-2Q_{4}$ (norm criterion) & [K] Thm~\ref{gamma_verdicts} \\
...
\texttt{n2Q4\_not\_square} [K].  The two $C_{\pm}$ covers thus share the single radicand
...
\texttt{terrain\_crosscheck}, \texttt{terrain\_class12}], so none is a constant
multiple of a square.  Degree four and integrality follow [M].
...
to $-2Q_{4}$ (same zero divisor).  For $C_{\pm}$, the layer-and-norm chain of
Theorem~\ref{gamma_verdicts}, read over $\overline{\Q}$ --- its inputs, $R_{q}$
nonsquare and the odd $Q_{4}$-divisor, hold over $\overline{\Q}$ by
Proposition~\ref{prop:S_integral} and the factorisation data [C] --- reduces
geometric squareness of $\gamma_{\pm}$ to that of $-2Q_{4}$, excluded above.  Dominance and generic degree two are
