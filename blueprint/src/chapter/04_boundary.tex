...
an empty domain.  This one line is the only known route to T0 (the adjudication record,
P1/P2/P7).
\end{proof}

\section{The terrain, machine-verified}
...
\begin{lemma}[Q4 factors into three rational curves]\label{Q4_factor}\lean{BMThm7Boundary.Q4_factor}\leanok
\uses{boundary-defs}
$Q_{4}=F_{1}\,F_{2}\,F_{3}$; squarefree, with $F_{1}$ symmetric ($=\mathrm{den}-1$) and
$F_{2}(a,b)=-F_{3}(b,a)$.  Each $F_{i}=0$ is an irreducible plane curve of genus~$0$.
\end{lemma}
\begin{proof}\leanok
By \texttt{ring}.
\end{proof}
...
The pairing identities identify $Q_{4}=0$ --- the locus where Theorem~7's conclusion is
automatic --- with the locus on which the quartic $\{1,a,b,c\}$ is symmetric under one of
the three root pairings, which is Buchholz--MacDougall's own excluded case~(v).  The
automatic locus and the excluded case coincide.
...
cover is \emph{negative} at an explicit rational point, kernel-anchoring the two-system
verdict that none of the eight is a square --- so $\sqrt{Q_{4}}\notin\Q(S)$ and
$\sqrt{-2Q_{4}}\notin\Q(S)$, and all three covers are irreducible nontrivial double
covers of $S$.  Point~1 is the $K$-witness $(-51/13,-17/7)$; point~2 is $(1,3)$.
...
curve $S\cap\{Q_{4}=0\}$: six one-dimensional components, each of genus~$0$.  On each
component $R_{q}$ restricts to a square while $S_{q}$ does not; the restriction of $S_{q}$
has constant square class $12=4\cdot 3$, so $w^{2}=S_{q}$ has rational solutions only
where $S_{q}=0$, and the only rational roots of the even part ($t=-1$, $t=0$) are
inadmissible.  Hence over $\Q$ the conclusion is never automatic: every hypothetical
nondegenerate point of $S(\Q)$ must lift through a nontrivial double cover, and the
Conjecture-1-refutation scenario via these curves is closed (no admissible points).
...
cover form a thin subset.  There is no geometric mechanism; twist classes on a surface are
unbounded, so there is no descent finiteness either; and the field-dependence begins
already at Buchholz--MacDougall's own excluded case, over the smallest field
$\Q(\sqrt{3})$ (since $12=4\cdot 3$), while Stroeker's points live over $\Q(\sqrt{3441})$.

The reading is machine-established but assessed, not proved: every escape hatch for a
Theorem-7 proof that avoids Conjecture~1 is shut at the level computation can shut it ---
no geometric mechanism, no automatic locus over $\Q$, no descent finiteness --- leaving
the single remaining route, that $S(\Q)$ has no nondegenerate points, which is
Conjecture~1 itself.  Every known route to Theorem~7 passes through the white node.
