...
\begin{definition}[The group $\langle X^{*}\rangle$]\label{def:affequiv}\lean{Thm7Classification.AffineEquiv}\leanok
...
\uses{def:naturalrd}
$f$ and its first three derivatives split over $\Q$.
\end{definition}
...
For a split $(2,1,1)$ quartic $f$: $f$ is rational-derived iff $f$ is
$\langle X^{*}\rangle$-equivalent to $Q_{a\mathrm{Map}(w,z)}$ for a rational
point $(w,z)$ of $E=\texttt{576i2}$ away from the denominator and exceptional
loci.
\end{theorem}
\begin{proof}\leanok
